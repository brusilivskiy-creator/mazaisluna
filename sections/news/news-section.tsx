...
export function NewsSection() {
  ...
  const [navigationItems, setNavigationItems] = useState<{ label: string; category: string | null }[]>([]);
  ...
  const fetchNavigationItems = async () => {
    try {
      const response = await fetch("/api/categories?type=news_navigation");
      const data = await response.json();
      const items = [
        { label: "Новини Уряду", category: null },
        ...data.map((cat: { name: string }) => ({
          label: cat.name,
          category: cat.name,
        })),
      ];
      ...
      setNavigationItems([
        { label: "Новини Уряду", category: null },
        { label: "Новини органів влади", category: "Новини органів влади" },
        { label: "Виступи та коментарі", category: "Виступи та коментарі" },
        { label: "Інфографіка", category: "Інфографіка" },
        { label: "Анонси", category: "Анонси" },
      ]);
    }
  };
  ...
  // Получаем уникальные категории из новостей
